...
public final class TypeHandlerRegistry {
  ...
  /**
   * javaType和(类型处理器和JdbcType映射表)之间的映射关系表
   * 比如java 中的String 类型 对应 数据库中的类型(char, varchar)
   */
  ...
  /**处理器Class对象和类型处理器映射关系表*/
  private final Map<Class<?>, TypeHandler<?>> allTypeHandlersMap = new HashMap<>();

  /**JdbcType和类型处理器映射关系表 ====> 空表   */
  ...
  private <T> TypeHandler<T> getTypeHandler(Type type, JdbcType jdbcType) {
    ...
    if (jdbcHandlerMap != null) {
      ...
      if (handler == null) {
        // #591
        //上面条件都不满足, 如果 jdbcHandlerMap 中只有一个 jdbcHandler 就获取
  ...
  private Map<JdbcType, TypeHandler<?>> getJdbcHandlerMap(Type type) {
    ...
    if (jdbcHandlerMap == null && type instanceof Class) {
      ...
      if (Enum.class.isAssignableFrom(clazz)) {
        //clazz.isAnonymousClass() 判断该类是否是匿名类
        Class<?> enumClass = clazz.isAnonymousClass() ? clazz.getSuperclass() : clazz;
        //查找已初始化的枚举接口的JdbcHandlerMap
  ...
  private <T> void register(Type javaType, TypeHandler<? extends T> typeHandler) {
    ...
    //判断类型处理器上是否使用了MappedJdbcTypes注解
